// frontend/src/pages/profile/private/Catalogo.jsx
...
const GestorCatalogos = () => {
  ...
  const publicarCatalogo = async () => {
    ...
    try {
      ...
      if (catalogoEditando) {
        await catalogoService.actualizarCatalogo(catalogoEditando.id, dataToSend);
  ...
  // Editar catálogo (desde el botón de editar en la tarjeta)
  ...
  // Ver detalle del catálogo (desde la tarjeta)
  const verDetalle = (catalogo) => {
    abrirDetalle(catalogo);
  };
  ...
  // Navegar entre imágenes en el modal de detalle
  ...
  return (
    <>
      <div className="container-fluid p-0">
        ...
        {/* MODAL CREAR/EDITAR CATÁLOGO - CONTROLADO CON STATE */}
        <Modal 
        ...
        >
          <div className="p-4 text-center text-white" style={{ background: 'linear-gradient(to right, #2a140a, #8d4925)' }}>
            <h2 className="fw-bold mb-1 fs-3">
              {catalogoEditando ? 'Editar Catálogo' : 'Crear Nuevo Catálogo'}
            </h2>
            <p className="mb-0 small opacity-75">
              {catalogoEditando ? 'Modifica los detalles de tu catálogo' : 'Agrupa tus mejores obras bajo un mismo concepto.'}
            </p>
          </div>
          ...
          <div className="modal-footer border-0 p-4 pt-0 d-flex justify-content-between">
            {paso === 1 ? (
              <>
                <button type="button" className="btn-linear-gradient py-2 px-4 rounded-pill" onClick={() => { resetFormulario(); setShowModalCatalogo(false); }}>
                  Cancelar
                </button>
              ...
              <>
                ...
                <div className="d-flex align-items-center gap-4">
                  ...
                  <button 
                    type="button" 
                    className="btn-linear-gradient py-2 px-4 rounded-pill text-white" 
                    onClick={publicarCatalogo}
                  >
                    {catalogoEditando ? 'Guardar Cambios' : 'Publicar Catálogo'} <i className="bi bi-check-lg ms-2"></i>
                  </button>
                </div>
              </>
            )}
        ...
        {/* MODAL DETALLE CATÁLOGO - CONTROLADO CON STATE SEPARADO */}
        <Modal 
