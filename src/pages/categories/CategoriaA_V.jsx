...
const extraerCantidadArticulos = (titulo) => {
    if (!titulo) return 0;
    // Busca "y X más" en el título
    const match = titulo.match(/y (\d+) más/);
    if (match) {
        return parseInt(match[1]) + 1; // +1 por el primer artículo
    }
    return 0;
};
...
const obtenerPortada = (subasta) => {
    ...
    // Lista de propiedades donde puede estar la imagen
